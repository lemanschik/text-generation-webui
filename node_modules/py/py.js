...
const getPythonDownloadLink = async (versionRange) => {
    var _a, e_1, _b, _c;
    const version = versionRange !== null && versionRange !== void 0 ? versionRange : '>= 0';
    const targetMap = {
        darwin: {
    ...
    try {
        for (var _d = true, releasesIterator_1 = __asyncValues(releasesIterator), releasesIterator_1_1; releasesIterator_1_1 = await releasesIterator_1.next(), _a = releasesIterator_1_1.done, !_a; _d = true) {
            _c = releasesIterator_1_1.value;
            _d = false;
            const { data: releases } = _c;
            for (const release of releases) {
                ...
                const asset = release.assets
                    .map((a) => { var _a, _b; return (Object.assign(Object.assign({}, a), { version: (_b = (_a = assetRegex.exec(a.name)) === null || _a === void 0 ? void 0 : _a.groups) === null || _b === void 0 ? void 0 : _b['version'] })); })
                    .sort((a, b) => {
        ...
        try {
            if (!_d && !_a && (_b = releasesIterator_1.return)) await _b.call(releasesIterator_1);
        }
 ...
 */
export const downloadPython = async (versionRange) => {
    ...
    if (matchingExistingVersion) {
        ...
        if (await fse.pathExists(process.platform === 'win32'
    ...
    if (!checksumUrl)
        throw new Error(`No checksum URL for Python ${version} found.`);
...
const isVenv = async (dir) => (await fse.pathExists(join(dir, 'pyvenv.cfg'))) &&
    (await fse.pathExists(process.platform === 'win32' ? join(dir, 'Scripts', 'python.exe') : join(dir, 'bin', 'python3')));
/**
 ...
 */
export const getVenv = async (options) => {
    var _a, _b, _c;
    const cacheDir = await globalCacheDir('autopy');
    ...
    if (options.pythonVersion && (await fse.pathExists(venvDir)) && (await isVenv(venvDir))) {
        ...
        const venvPythonVersion = (_b = (_a = pyvenvCfg
            .split('\n')
            .find((l) => l.startsWith('version = '))) === null || _a === void 0 ? void 0 : _a.split('=')[1]) === null || _b === void 0 ? void 0 : _b.trim();
        if (venvPythonVersion && !semverSatifies(venvPythonVersion, options.pythonVersion)) {
            if (options.allowPythonUpgrade !== false)
                await fse.remove(venvDir);
            else
                throw new Error(`The virtual environment "${options.name}" already exists, but has a different Python version (${venvPythonVersion}) than requested (${options.pythonVersion}) and upgrading is not allowed.`);
        }
    }
    if (!(await fse.pathExists(venvDir)) || !(await isVenv(venvDir))) {
        const pythonDir = await downloadPython(options.pythonVersion);
        const globalPythonBinary = process.platform === 'win32' ? join(pythonDir, 'python.exe') : join(pythonDir, 'bin', 'python3');
        await execa(globalPythonBinary, ['-m', 'venv', venvDir]);
    }
    const venvPythonBinary = process.platform === 'win32' ? join(venvDir, 'Scripts', 'python.exe') : join(venvDir, 'bin', 'python3');
    if (options.checkRequirements !== false) {
        ...
        // first.
        const pipVersion = (_c = options.requirements.find((r) => r.name === 'pip')) === null || _c === void 0 ? void 0 : _c.version;
        if (pipVersion)
            await execa(venvPythonBinary, ['-m', 'pip', 'install', `pip${pipVersion}`]);
        ...
        const missingPackages = options.requirements.filter((r) => r.name !== 'pip' &&
    ...
    return (file, args, options) => execa(file, args, Object.assign(Object.assign({}, options), { env: Object.assign({ PYTHONHOME: undefined, VIRTUAL_ENV: venvDir, PATH: `${venvBinDir}${process.platform === 'win32' ? ';' : ':'}${process.env['PATH']}` }, options === null || options === void 0 ? void 0 : options.env) }));
};
/**
 ...
 */
export const removeVenv = async (name) => remove(join(await globalCacheDir('autopy'), 'venv', name));
 ...
 * @returns The path to the Python installation directory.
 */
// export declare const downloadPython: (versionRange: SemverVersionSpecifier) => Promise<string>;
 ...
 *   [`execa`](https://github.com/sindresorhus/execa).
 */
//export declare const getVenv: (options: VenvOptions) => Promise<(file: string, args?: string[], options?: ExecaOptions) => import("execa").ExecaChildProcess<string>>;
/**
 * Removes the virtual environment with the specified name.
 *
 * @param name The name of the virtual environment to remove.
 */
//export declare const removeVenv: (name: string) => Promise<void>;
